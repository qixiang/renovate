...
function parseLockFile(content: string): RpmLockfile | null {
  try {
    return parseSingleYaml(content) as RpmLockfile;
  } catch {
    return null;
  }
}
...
export async function extractAllPackageFiles(
  ...
  // Clear caches at the start of extraction
  clearDetectedUpdates();
  clearNewLockFileCache();
  clearOriginalInputFileCache();
  ...
  for (const packageFileName of packageFiles) {
    ...
    const result = await extractSinglePackageFile(packageFileName);
    if (result) {
      results.push({
        ...result,
...
async function extractSinglePackageFile(
  ...
  logger.debug({ packageFileName }, 'rpm-lockfile.extractPackageFile()');
  ...
  logger.debug(
    { packageFileName, lockFileName, inputFileName, dir },
    'rpm-lockfile paths',
  );
      ...
      if (newLockfile) {
        ...
        if (updates.length) {
          ...
          for (const update of updates) {
            setDetectedUpdate(lockFileName, update.name, update.newEvr);
            logger.debug(
              {
                lockFileName,
                package: update.name,
